    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{llllllll}
            ...
            \multirow{14}{*}{Termopara}
                    ...
                    & Łączone    & wszystkie & $T = aU + T_0$             & Tak ($T_0 = 0\,^\circ\mathrm{C}$) & $\num{24}   \pm \num{0.14}$     & ---                             \\
                    & Łączone    & wszystkie & $T = aU + b$               & Nie                               & $\num{21}   \pm \num{0.14}$     & $\num{6.8} \pm \num{0.35}$      \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
% Termopara: regresja na wszystkich punktach łącznie
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{wykres5_termopara_wszystkie.pdf}
    \caption{Regresja liniowa termopary wyznaczona na podstawie wszystkich punktów pomiarowych
        (grzanie i chłodzenie łącznie).
        Oś~$x$: napięcie termoelektryczne $U$ (mV); oś~$y$: temperatura $T$ (\si{\degreeCelsius}).
        Linia przerywana (niebieska) --- model zakotwiczony $T = aU + T_0$
        z $T_0 = 0\,^\circ\mathrm{C}$; zacieniowany obszar --- $u(\hat{T}) = |U|\cdot u(a)$.
        Linia ciągła (czerwona) --- model swobodny $T = aU + b$;
        zacieniowany obszar --- $u(\hat{T}) = \sqrt{{(U\cdot u(a))}^2 + {u(b)}^2}$.
        Połączenie wszystkich danych minimalizuje niepewności współczynników regresji
        i daje globalną charakterystykę termopary.}\label{fig:termopara_wszystkie}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{l
        ...
        Łączone --- wszystkie     & 23.76                                                             & 0.14 & 0.04209 & 0.00025 & 42.09 & 0.25 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{l
        ...
        Łączone --- wszystkie     & 21.22                                                             & 0.14  & 0.04713 & 0.00032 & 47.13 & 0.32 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{l
        ...
        \midrule
        Zakotwiczony & 0.04108                                                                 & 0.00009 \\
        Swobodny     & 0.04716                                                                 & 0.00011 \\
        \bottomrule
    \end{tabular}
\end{table}

Porównując wyniki ze wszystkich modeli można zauważyć, że wartości współczynnika Seebecka nie są w granicach swoich niepewności. Prowadzi to do wniosku, że różne części ogrzewania i chłodzenia mogły być obarczone różynymi błędami systematycznymi. Z tego powodu odrzucono wyniki obliczone ze średnich i jako ostateczny wynik przyjęto wyniki obliczone metodoą regresji ze wszystkich punktów chłodzenia i ogrzewania termopary. Wyniki te przestawiono w tabeli~\ref{tab:alpha_wynik_final}.
\begin{table}[H]
    \centering
    \sisetup{separate-uncertainty=false}
    \caption{Ostateczne wyniki współczynnika Seebecka $\alpha$ termopary dla obu modeli regresji, obliczone na podstawie wszystkich punktów pomiarowych.}\label{tab:alpha_wynik_final}
    \setlength{\tabcolsep}{12pt}
    \begin{tabular}{l
            S[table-format=1.5] @{${}\pm{}$} S[table-format=1.5]}
        \toprule
        Model
                     & \multicolumn{2}{c}{$\alpha$ (\si{\milli\volt\per\degreeCelsius})}          \\
        \midrule
        Zakotwiczony & 0.04209                                                                 & 0.00025 \\
        Swobodny     & 0.04713                                                                 & 0.00032 \\
        \bottomrule
    \end{tabular}
\end{table}
% -------------------------------------------
...
Wyniki z różnych regresji dla termopary nie są zgodne w granicach niepewności. Na wykresie jasno widać że punkty w różnych odcinkach znajdowały się nad lub pod prostą regresji. Prowadzi to do wniosku, że w trakcie wykonywnania pomiarów w różnych momentach występowały różne błędy systematyczne. Z tego powodu odrzucono wyniki obliczone jako średnie ważone z poszczególnych regresji a jako ostateczny wynik przyjęto współczynnik Seebecka $\alpha = \SI{0.04209}{\milli\volt\per\degreeCelsius} \pm \SI{0.00025}{\milli\volt\per\degreeCelsius}$, wyznaczony z modelu zakotwiczonego $T = aU + T_0$ na podstawie wszystkich punktów pomiarowych. Otrzymany wynik jest zgodny z typowymi wartościami współczynnika Seebecka dla termopar typu K, które wynoszą około $\SI{0.041}{\milli\volt\per\degreeCelsius}$.
\end{document}
% =============================================

...
Przy ustawieniu ćwierćfalówki pod kątem $\theta_\text{rel} = 45^{\circ}$ do polaryzatora
uzyskano eliptyczność doświadczalną $e_\text{exp} = 0{,}195 \pm 0{,}012$, znacząco
odbiegającą od wartości teoretycznej $e_\text{teor} = 1{,}000 \pm 0{,}049$
dla polaryzacji kołowej. Na rozbieżność składają się dwa efekty.
Po pierwsze, fluktuacje wskazań miernika sięgające ponad $10\,\%$ odczytu
powodują, że parametry $a_1$ i $a_3$ wyznaczone z dopasowania obarczone są
niepewnością znacznie większą niż sugeruje macierz kowariancji --- $e_\text{exp}$
może być przeszacowana lub niedoszacowana zależnie od kierunku fluktuacji
w chwili pomiaru.
Po drugie, parametr dopasowania $a_2 = (10{,}0 \pm 0{,}5){}^{\circ}$ wskazuje,
że główna oś elipsy jest obrócona o $10^{\circ}$ względem osi polaryzatora.
Dla idealnej polaryzacji kołowej ($e = 1$) kierunek osi głównej jest nieokreślony;
niezerowe $a_2$ świadczy zatem o tym, że układ produkuje polaryzację
eliptyczną, a nie kołową, nawet abstrahując od wartości $e_\text{exp}$.
Obrót osi o $10^{\circ}$ jest typowym sygnałem błędnego ustawienia kąta ćwierćfalówki
($\theta_\text{rel} \neq 45^{\circ}$), a nie wyłącznie nieidelaności optycznej.
Po trzecie, zastosowana ćwierćfalówka może nie być idealną płytką $\lambda/4$
dla długości fali $\lambda = \SI{578}{nm}$ --- nieidealne opóźnienie fazowe
$\delta \neq \pi/2$ również zmniejsza eliptyczność poniżej wartości kołowej.
Obie przyczyny (błąd kąta ustawienia i nieidealne $\delta$) mogą działać jednocześnie.
Pomimo rozbieżności cos$^2$-kształt krzywej z niezerowym minimum potwierdza
zmianę stanu polaryzacji przez ćwierćfalówkę.
